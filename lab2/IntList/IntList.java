...
public class IntList {
    ...
    public int size() {
        if (rest == null) {//迭代到最后一个结点，即rest = null
            return 1;
        }
        return 1 + this.rest.size();//逐层迭代，类似二叉树中序遍历
    }

    /** Return the size of the list using no recursion! */
    public int iterativeSize() {
        IntList p = this;//用p结点代替整个链表的开头
        int totalSize = 0;
        while (p != null) {
            totalSize += 1;
            p = p.rest;//结点后移
    ...
    public static IntList of(int ...argList) {
        if (argList.length == 0)//如果没有传递任何参数
            return null;
